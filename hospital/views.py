...
def appointment(request):
    ...
    # Get doctors who are available (for example, no scheduled appointments)
    available_doctors = models.doctor_infos.objects.exclude(id=user)  # Exclude the logged-in doctor

    # Check for any scheduled appointments for the patient with the selected doctor
    if request.method == 'POST':
        if request.POST.get('accept'):
            ...
            message = "Hi {}, your appointment has scheduled on {}. For more details contact through this hospital mail or doctor via phone {}".format(patient_name, appointment_date, doctor_phone_number)
            ...
            message = "Hi {}, your appointment has been cancelled for the date {}. Please try again later or contact the doctor for rescheduling.".format(patient_name, appointment_date)
    ...
    context = {'patient': patient, 'scheduled': scheduled, 'today': today, 'future': future, 'available_doctors': available_doctors}
...
def add_doctor(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('doctor_list')  # Redirect to the doctor list page (you may want to define this)
        else:
            messages.error(request, 'There was an error adding the doctor. Please check the form.')
    else:
        form = doctor_infos()  # Create an empty form for GET requests
